package am_study3.src.command.copy.copy;
...
public class main_cmd {
	...
	public static void main(String[] args) {
		...
		while(it.hasNext()){
			venture temp = it.next(); // ArrayList의에 저장된 기업객체 하나씩 temp에 저장
		...
		// 그러므로 위와 같이  검색하고자하는 회사이름을 따로 입력해서 객체를 생성하여 검색에 사용하는방식은
...
////				"박영주 어그", list.get(0).getCategory(),list.get(0).getBusiness_name(),
////				list.get(0).getProduct());
//		
...
//		while(test.hasNext()) { // name1 의 데이터를 hab에 저장
//			hab.add(test.next());
//		}
//		test=name2.iterator();
